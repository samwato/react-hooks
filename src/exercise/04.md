...
- Not everthing has to be in React.useState, it can be derived state that gets calucated in some way of of useState.
- const initial state values can be useful to initalise state and reset it. It does not have to be spread to the state. The rule of not mutating only applies to useState state.
- slice was handy to return a new array
