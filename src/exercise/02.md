...
- you can do logic for variables to pass into useState before useState is declared
- did not know about lazy initialization for useState, where a function is passed as the init value of useState()
- good use always use dep render, incase parent component re-renders
- react custom hooks start with 'use*' and are simply a function that call React hooks or other custom hooks. Used to abstract logic making it reusable.
